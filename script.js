...
// API Call Function for Puter
async function callPuterAPI(messages, isStreaming = false) {
  ...
  if (!response.ok) {
    throw new Error("Error with Puter API: " + response.statusText);
  }
...
async function handleNonStreaming(message) {
  const messages = [{ role: "user", content: message }];
  try {
    const response = await callPuterAPI(messages);
    ...
    console.error("Error in non-streaming API call:", error);
  }
}
...
async function handleStreaming(message) {
  const messages = [{ role: "user", content: message }];
  try {
    const response = await callPuterAPI(messages, true);
...
async function processAIResponse(response, aiDiv) {
  ...
  try {
    if (response && typeof response[Symbol.asyncIterator] === "function") {
      ...
      aiResponse =
        response?.text || response?.message?.content || "No content available.";
    }
    return aiResponse;
  } catch (error) {
...
const applyCustomModelButton = document.getElementById(
  "apply-custom-model-button"
);
...
function initializeUI() {
  ...
  settingsButton.addEventListener("click", () =>
    showModal(UI_CONSTANTS.MODAL_IDS.SETTINGS)
  );
  ...
  historyButton.addEventListener("click", () =>
    showModal(UI_CONSTANTS.MODAL_IDS.HISTORY)
  );
  closeHistoryModal.addEventListener("click", closeAllModals);
  newChatButton.addEventListener("click", startNewChat);

  toolsButton.addEventListener("click", () =>
    showModal(UI_CONSTANTS.MODAL_IDS.TOOLS)
  );
  ...
  webSearchButton.addEventListener("click", () =>
    showModal(UI_CONSTANTS.MODAL_IDS.WEB_SEARCH)
  );
...
function toggleCustomModelCard() {
  customModelCard.style.display =
    customModelCard.style.display === "none" ? "block" : "none";
}
...
async function handleCustomModel(message, model) {
  try {
    ...
    return {
      message: { content: [{ text: data.choices[0].message.content }] },
    }; // Consistent response
  } catch (error) {
    console.error("Failed to get response from Custom Endpoint", error);
    return {
      text: "Failed to get response from Custom Endpoint: " + error.message,
    };
  }
}
...
function loadChat(chatId = null) {
  ...
  if (currentChat) {
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      messageDiv.innerHTML = `<span class="message-timestamp">${
        msg.timestamp || getTimestamp()
      }</span> <span class="message-prefix">${
        msg.role === "user" ? "You: " : "AI: "
      }</span>${msg.content.replace(/\n/g, "<br>")}`;
